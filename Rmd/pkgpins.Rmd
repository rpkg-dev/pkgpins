...
-   Figure out how to properly pass on `...` to `fn()`; Currently only works with lamda fns, not with "normal" non-lamda fns:
...
#'
#' @param .pkg The package name to which `.fn` belongs. A character scalar.
#' @param .fn_name The name of the newly created function, i.e. the name for the function that is returned by `cachely()`. A character scalar.
...
cachely <- function(.pkg,
  ...
  rlang::new_function(args = rlang::fn_fmls(fn),
                      body = quote(expr = with_cache(expr = do.call(what = fn,
