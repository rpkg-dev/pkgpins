...
- Find out why retrieving an obj with a "structure"-name won't work...
...
- The `args` are not properly evaluated, i.e. the search path is "cut off". Example where this matters:
    ...
    Maybe thoroughly read the chapter [Environments](https://adv-r.hadley.nz/environments.html) in the _Advanced R_ book the better understand the whole
    situation?
...
- Provide an even shorter way:
  ...
  which returns a function, similar to `purrr::safely()`. Maybe name it `cachely()` instead? 🥳
