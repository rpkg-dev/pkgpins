...
-   tidyverse devs [discourage](https://github.com/tidyverse/magrittr/blob/master/NEWS.md#incorrect-call-stack-introspection) the use of `sys.parent()`,
    `sys.frame()` and friends and recommend to only rely on `parent.frame()` since it also works when R code is called from *non-inspectable* frames. See
    `?rlang::stack` for more information on the call vs. the context stack in R.
