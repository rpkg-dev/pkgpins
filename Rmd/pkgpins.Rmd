...
- The `args` are not properly evaluated, i.e. the search path (or "env stack" or whatever) is "cut off". Real-life example where this matters:
    ...
    Maybe thoroughly read the chapter [Environments](https://adv-r.hadley.nz/environments.html) of the _Advanced R_ book to better understand the whole
    situation?
    
    NOTES:
    
    - As it seems, the current way might be a dead end because we "break" R's concept of [promises](https://adv-r.hadley.nz/functions.html#promises) by trying
      to reconstruct the args from `match.call()`... (is this really true?)
